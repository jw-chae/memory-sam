# Memory-SAM Submit Version

This folder contains the user-facing MICCAI submit/release version of Memory-SAM:

> Memory-Augmented Retrieval-to-Prompt for Training-Free Tongue Segmentation

The package keeps the paper-aligned Memory-SAM core and a practical Gradio UI. It does not ship with a prebuilt memory bank. Users create their own memory items from reference images and masks through the UI.
...
After assets are prepared, the submit folder should contain:
...
Do not commit these folders to GitHub. They are ignored by `mt_sam_submit/.gitignore`:

```text
mt_sam_submit/assets/dinov3_weights/
mt_sam_submit/checkpoints/
mt_sam_submit/third_party/
mt_sam_submit/configs/
mt_sam_submit/user_memory/
mt_sam_submit/results/
```
...
```text
mt_sam_submit/checkpoints/sam2.1_hiera_large.pt
```

and clones SAM2 code into:

```text
mt_sam_submit/third_party/sam2/
```
...
```text
mt_sam_submit/assets/dinov3_weights/dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth
```
...
python mt_sam_submit/scripts/setup_assets.py --copy
```

Use symlinks instead of copies when the filesystem supports them:

```bash
python mt_sam_submit/scripts/setup_assets.py
```
...
```bash
python mt_sam_submit/scripts/infer_image.py \
  --image /abs/path/query.png \
  --memory-dir mt_sam_submit/user_memory \
  --sam-checkpoint mt_sam_submit/checkpoints/sam2.1_hiera_large.pt \
  --sam-config configs/sam2.1/sam2.1_hiera_l \
  --out-mask mt_sam_submit/results/query_mask.png \
  --out-meta mt_sam_submit/results/query_meta.json \
  --device cuda
```
...
```bash
python mt_sam_submit/scripts/evaluate_split.py \
  --manifest /abs/path/test.csv \
  --memory-dir mt_sam_submit/user_memory \
  --sam-checkpoint mt_sam_submit/checkpoints/sam2.1_hiera_large.pt \
  --sam-config configs/sam2.1/sam2.1_hiera_l \
  --out-dir mt_sam_submit/results/eval \
  --device cuda
```
